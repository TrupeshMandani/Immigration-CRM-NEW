...
const StudentDetail = () => {
  ...
  const fetchStudentTasks = useCallback(async () => {
    ...
    try {
      ...
      const list = await studentTaskService.list(studentAiKey);
  ...
  const handleCreateTask = async (event) => {
    ...
    setTaskError("");
    try {
      const list = await studentTaskService.create(studentAiKey, {
  ...
  const handleDownloadTaskAttachment = async (task) => {
    ...
    try {
      const url = await studentTaskService.attachmentUrl(studentAiKey, task.id);
      if (url) {
  ...
  const handleDeleteTask = async (task) => {
    ...
    setTaskError("");
    try {
      const list = await studentTaskService.delete(studentAiKey, task.id);
      setStudentTasks(list);
    } catch (error) {
