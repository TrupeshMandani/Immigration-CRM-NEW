...
3. ✅ You have a valid student account with an `aiKey`
4. ✅ You have authentication token (Bearer token)

## Getting Test Data

### 1. Get a Student's AI Key
```bash
# Login as admin and get a student record
...
curl -X GET http://localhost:4000/api/students/{aiKey}/required-documents \
...
2. Say: *"Upload the file /path/to/test-passport.pdf for student KEY-123's passport requirement DOC-456"*
...
```bash
curl -X POST http://localhost:4000/api/students/{aiKey}/required-documents/{docId}/files \
  -H "Authorization: Bearer YOUR_TOKEN" \
...
```json
{
  ...
  "data": {
    "file": {
      ...
      "key": "student-key/required-docs/passport/test-passport.pdf"
    }
  }
}
```
...
## Test Case 2: getStudentDocuments

### Purpose
Retrieve all documents for a student with presigned URLs

### Test Steps

**Via AI Assistant UI:**
1. Ask: *"Show me all documents for student KEY-123"*
2. AI calls `getStudentDocuments` tool
...
```bash
curl -X GET http://localhost:4000/api/students/{aiKey}/files \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
### Validation Checklist
- [ ] All student documents are listed
...
**Via AI Assistant UI:**
1. Ask: *"Get the presigned URL for file FILE-123 in document DOC-456 for student KEY-789"*
2. AI calls `getDocumentById` tool
3. Verify URL is returned

**Via cURL (Backend Direct):**
```bash
curl -X GET "http://localhost:4000/api/students/{aiKey}/required-documents/{docId}/files/{fileId}/url" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
1. Ask: *"Verify file FILE-123 in document DOC-456 for student KEY-789 as approved with note 'Passport verified'"*
2. AI calls `verifyDocument` tool
3. Verify success response

**Via cURL (Backend Direct):**
```bash
curl -X POST "http://localhost:4000/api/students/{aiKey}/required-documents/{docId}/files/{fileId}/verify" \
...
- [ ] Only admin can verify (test with student token - should fail)
...
**Via AI Assistant UI:**
1. Ask: *"Rename document DOC-123 for student KEY-456 to 'passport-updated.pdf'"*
2. AI calls `renameDocument` tool
3. Verify document name changes

**Via cURL (Backend Direct):**
```bash
curl -X POST "http://localhost:4000/api/students/{aiKey}/documents/{documentId}/rename" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
**Via AI Assistant UI:**
1. Ask: *"Delete document DOC-123 for student KEY-456"*
2. AI calls `deleteDocument` tool
3. Verify document is removed

**Via cURL (Backend Direct):**
```bash
curl -X DELETE "http://localhost:4000/api/students/{aiKey}/documents/{documentId}" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
**3. Invalid Student Key**
```javascript
await getStudentDocuments({ aiKey: "invalid-key-999" });
// Expected: HTTP 404 or "Student not found"
```

**4. Unauthorized Access (Student trying admin operation)**
```javascript
...
```bash
# 1. Get student documents (should be empty or have old docs)
"Show all documents for student ABC-123"

# 2. Upload a new document
"Upload /tmp/test-passport.pdf for student ABC-123's passport requirement"
...
"Get the URL for the passport file for student ABC-123"

# 5. Rename the document
"Rename the passport document to 'passport-final.pdf' for student ABC-123"

# 6. Verify document (as admin)
"Verify the passport document for student ABC-123 as approved"
...
# 8. Delete document
"Delete the passport document for student ABC-123"
...
[INFO] Document uploaded successfully for student-123/passport-req
...
📁 Fetching files for student ABC-123: 3 documents found
...
AI_KEY="YOUR_STUDENT_KEY"
DOC_ID="YOUR_DOC_ID"

# Test 1: Get all documents
echo "Test 1: Getting all documents..."
curl -X GET "$BASE_URL/api/students/$AI_KEY/files" \
  -H "Authorization: Bearer $TOKEN"

# Test 2: Upload document
echo -e "\n\nTest 2: Uploading document..."
echo "Test content" > /tmp/test-doc.pdf
curl -X POST "$BASE_URL/api/students/$AI_KEY/required-documents/$DOC_ID/files" \
  -H "Authorization: Bearer $TOKEN" \
...
| "Student not found" | Invalid aiKey | Check student record in DB |
...
2. Verify with multiple students
