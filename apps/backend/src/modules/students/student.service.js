...
async function upsertStudent(data) {
  ...
  const [existing] = await db.select().from(students).where(eq(students.ai_key, aiKey)).limit(1);

  if (!profile || !Object.keys(profile).length) {
    if (!existing) throw new Error(`Student with aiKey ${aiKey} not found`);
    return existing;
  }

  if (!existing) throw new Error(`Student with aiKey ${aiKey} not found — cannot upsert profile`);
